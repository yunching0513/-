<picture>
  ...
  <img src="brand/logo-wordmark.svg" alt="Flowmato" width="330">
</picture>
...
| **神經相位鎖定**<br>大腦振盪會與外部節奏同步；有歌詞的音樂反而搶走專注資源 | **兩層音景**（可疊加、各自開關）：<br>① **專注音樂** — Beta 分析／Alpha 吸收／Theta 發散三首樂曲，自動無縫循環（音檔缺席時退回合成的雙耳節拍）<br>② **和聲鋪底** — 無旋律的 ambient 織體，專注時小調、休息時轉大調<br>休息時音樂自動切 Alpha |
| **一次只扛一件事**<br>工作記憶容量有限 | **待辦事項**：設定「本回合目標」顯示在計時器上方，完成的蕃茄自動記在該任務身上 |
| **先估再做**<br>估計與實際的落差是最可靠的自我校準訊號 | **規劃顆數**：每件事可規劃 1–8 顆蕃茄，圓點實心＝已完成、空心＝還欠；做超過會標出來。清單頂端把還欠的顆數換算成小時，避免一天排進做不完的計畫 |
...
- **規劃顆數** — 點第幾個圓點就是規劃幾顆蕃茄；計時器上方即時顯示「已完成／規劃」，休息畫面告訴你這件事還差幾顆
- **思緒卸載盒** — 開始前清空工作記憶，內容存在本機可回顧
- **休息鎖定 + 呼吸引導** — 低認知負荷的 DMN 恢復畫面
- **兩層音景** — 專注音樂（Beta 分析／Alpha 吸收／Theta 發散三首樂曲，自動無縫循環）＋ ambient 和聲鋪底，可任意疊加，`A` 鍵一鍵靜音
- **選單列即時倒數**（macOS App）— 視窗關掉照樣跑，menu bar 直接操作
- **完成通知＋提示音** — 柔和雙音鐘聲，可獨立開關
- **置頂小視窗**（macOS App）— 釘在所有視窗最上層
- **自動深淺色** — 三種模式各有專屬色（專注・陶紅／短休息・青綠／長休息・暮藍）
- **中英雙語** — 預設跟著系統語言，設定裡可手動切換；`ui/i18n.js` 用中文原字串當 key，翻不到就退回中文，不會出現空白
- **iPad 版面** — 螢幕夠大時環與字級整組放大、面板改為置中對話框；分割檢視縮窄會自動退回手機版面
- **品牌識別** — 標記把蕃茄與倒數環畫成同一個圓，`brand/` 有淺色深色的 SVG 與 PNG（`npm run logo` 可重新產生）
- **工作歷程** — 月曆檢視每天的專注濃度，點任一天看當天做了什麼、幾點到幾點、投入在哪件事
- **待辦與歷程同一份資料** — 每件事都綁一個日期。待辦清單新增，歷程當天就看得到；在歷程往回補記，清單也會出現在「先前未完成」。勾選、改規劃、設本回合目標，兩邊即時同步
- **當天實際花在哪** — 歷程裡每件事底下直接列出它當天的專注時段（`09:05–09:30 · 09:35–10:00`），規劃與實際擺在一起看
- **匯出到日曆** — 一鍵產生 `.ics`，每段專注是一個日曆事件，可匯入 Google／Apple／Outlook
- **今日統計** — 完成幾顆蕃茄、專注幾分鐘
- **人生天數** — 設定裡填生日與想活到幾歲（50–120），歷程頁顯示已經活了幾天、還剩幾天與進度
...
│   ├── i18n.js           中英字典與語言切換（字典 key 就是中文原字串）
